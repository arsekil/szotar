          ...
          <div className={styles.entryMain}>
            <div className={styles.entryContainer}>
              ...
              <div className={styles.entryContainerNevek}>
                <div className={styles.entryContainerBekuldo2}>
                  <div style={{ paddingRight: "5px" }}>Beküldő:</div>
                  <div>
                    {entries[currentIndex].bekuldo2
                      ? entries[currentIndex].bekuldo2
                      : "Törölt Tag"}
                  </div>
                </div>
                <div className={styles.entryContainerMagyarazo2}>
                  <div style={{ paddingRight: "5px" }}>Magyarázó:</div>
                  <div>
                    {entries[currentIndex].magyarazo2
                      ? entries[currentIndex].magyarazo2
                      : "Törölt Tag"}
                  </div>
                </div>
                <div className={styles.entryContainerDatum2}>
                  <div style={{ paddingRight: "5px" }}>Dátum:</div>
              ...
              <div className={styles.entryContainerMagy}>
                {entries[currentIndex].magy
                  ? entries[currentIndex].magy
                  : "Törölt Tag"}
              </div>
              <div className={styles.entryContainerNevek}>
                <div
                  className={styles.entryContainerMenuHome}
                  onClick={handleHome}
                >
                  Főoldal
                </div>
                ...
                <div
                  className={styles.entryContainerMenuStat}
                  onClick={handleAbout}
                >
                  Rólunk
                </div>
...
export async function getStaticPaths() {
  ...
  const firstLetters = [
    ...new Set(szotar.map((entry) => entry.halan.charAt(0).toLowerCase())),
  ];
...
export async function getStaticProps({ params }) {
  ...
  const entries = szotar.filter(
    (entry) =>
      entry.halan.charAt(0).toLowerCase() === params.letter.toLowerCase()
  );
